...
public class AuthClientsAPI: APIBase {
  ...
  "first" : true
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    public class func updateClient(clientKey clientKey: String, clientResource: ClientResource? = nil, completion: ((error: ErrorType?) -> Void)) {
    ...
    /**
     ...
     - returns: RequestBuilder<Void> 
     */
    public class func updateClientWithRequestBuilder(clientKey clientKey: String, clientResource: ClientResource? = nil) -> RequestBuilder<Void> {
        var path = "/auth/clients/{clientKey}"
        path = path.stringByReplacingOccurrencesOfString("{client_key}", withString: "\(clientKey)", options: .LiteralSearch, range: nil)
        let URLString = SwaggerClientAPI.basePath + path
        let parameters = clientResource?.encodeToJSON() as? [String:AnyObject]
