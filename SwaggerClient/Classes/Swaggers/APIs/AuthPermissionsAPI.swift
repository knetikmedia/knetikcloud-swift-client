...
public class AuthPermissionsAPI: APIBase {
  ...
  "first" : true
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    public class func updatePermission(permission permission: String, permissionResource: PermissionResource? = nil, completion: ((error: ErrorType?) -> Void)) {
    ...
    /**
     ...
     - returns: RequestBuilder<Void> 
     */
    public class func updatePermissionWithRequestBuilder(permission permission: String, permissionResource: PermissionResource? = nil) -> RequestBuilder<Void> {
        var path = "/auth/permissions/{permission}"
        path = path.stringByReplacingOccurrencesOfString("{permission}", withString: "\(permission)", options: .LiteralSearch, range: nil)
        let URLString = SwaggerClientAPI.basePath + path
        let parameters = permissionResource?.encodeToJSON() as? [String:AnyObject]
