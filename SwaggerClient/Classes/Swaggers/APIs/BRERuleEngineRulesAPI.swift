...
public class BRERuleEngineRulesAPI: APIBase {
  ...
  "first" : true
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    public class func updateBRERule(id id: String, breRule: BreRule? = nil, completion: ((error: ErrorType?) -> Void)) {
    ...
    /**
     ...
     - returns: RequestBuilder<Void> 
     */
    public class func updateBRERuleWithRequestBuilder(id id: String, breRule: BreRule? = nil) -> RequestBuilder<Void> {
        var path = "/bre/rules/{id}"
        path = path.stringByReplacingOccurrencesOfString("{id}", withString: "\(id)", options: .LiteralSearch, range: nil)
        let URLString = SwaggerClientAPI.basePath + path
        let parameters = breRule?.encodeToJSON() as? [String:AnyObject]
