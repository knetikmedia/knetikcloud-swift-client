...
public class BRERuleEngineTriggersAPI: APIBase {
  ...
  "first" : true
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    public class func updateBRETrigger(eventName eventName: String, breTriggerResource: BreTriggerResource? = nil, completion: ((error: ErrorType?) -> Void)) {
    ...
    /**
     ...
     - returns: RequestBuilder<Void> 
     */
    public class func updateBRETriggerWithRequestBuilder(eventName eventName: String, breTriggerResource: BreTriggerResource? = nil) -> RequestBuilder<Void> {
        var path = "/bre/triggers/{eventName}"
        path = path.stringByReplacingOccurrencesOfString("{event_name}", withString: "\(eventName)", options: .LiteralSearch, range: nil)
        let URLString = SwaggerClientAPI.basePath + path
        let parameters = breTriggerResource?.encodeToJSON() as? [String:AnyObject]
