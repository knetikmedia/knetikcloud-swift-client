...
public class GamificationTriviaAPI: APIBase {
  ...
  "published_date" : 123456789
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    public class func updateQuestionTemplate(id id: String, questionTemplateResource: QuestionTemplateResource? = nil, completion: ((error: ErrorType?) -> Void)) {
    ...
    /**
     ...
     - returns: RequestBuilder<Void> 
     */
    public class func updateQuestionTemplateWithRequestBuilder(id id: String, questionTemplateResource: QuestionTemplateResource? = nil) -> RequestBuilder<Void> {
        var path = "/trivia/questions/templates/{id}"
        path = path.stringByReplacingOccurrencesOfString("{id}", withString: "\(id)", options: .LiteralSearch, range: nil)
        let URLString = SwaggerClientAPI.basePath + path
        let parameters = questionTemplateResource?.encodeToJSON() as? [String:AnyObject]
