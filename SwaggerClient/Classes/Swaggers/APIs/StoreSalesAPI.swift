...
public class StoreSalesAPI: APIBase {
  ...
  "first" : true
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    public class func updateCatalogSale(id id: Int32, catalogSale: CatalogSale? = nil, completion: ((error: ErrorType?) -> Void)) {
    ...
    /**
     ...
     - returns: RequestBuilder<Void> 
     */
    public class func updateCatalogSaleWithRequestBuilder(id id: Int32, catalogSale: CatalogSale? = nil) -> RequestBuilder<Void> {
        var path = "/store/sales/{id}"
        path = path.stringByReplacingOccurrencesOfString("{id}", withString: "\(id)", options: .LiteralSearch, range: nil)
        let URLString = SwaggerClientAPI.basePath + path
        let parameters = catalogSale?.encodeToJSON() as? [String:AnyObject]
