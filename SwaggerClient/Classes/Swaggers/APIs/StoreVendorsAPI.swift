...
public class StoreVendorsAPI: APIBase {
  ...
  "first" : true
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    public class func updateVendor(id id: Int32, vendor: VendorResource? = nil, completion: ((error: ErrorType?) -> Void)) {
    ...
    /**
     ...
     - returns: RequestBuilder<Void> 
     */
    public class func updateVendorWithRequestBuilder(id id: Int32, vendor: VendorResource? = nil) -> RequestBuilder<Void> {
        var path = "/vendors/{id}"
        path = path.stringByReplacingOccurrencesOfString("{id}", withString: "\(id)", options: .LiteralSearch, range: nil)
        let URLString = SwaggerClientAPI.basePath + path
        let parameters = vendor?.encodeToJSON() as? [String:AnyObject]
